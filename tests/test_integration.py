...
Covered scenarios:
  ...
  - DataLoader + Model end-to-end (on-disk dataset → DataLoader → BEVFormerModel)
"""
...
# ---------------------------------------------------------------------------
# 8. DataLoader + Model integration (on-disk dataset)
# ---------------------------------------------------------------------------
...
def _write_dataset_frame(
    ...
    """Write a synthetic JSON frame and a small PNG image to *root* for dataset tests."""
    cam_name = "front"
    img_rel_path = f"images/{cam_name}/{frame_id}.png"
    img_dir = root / "images" / cam_name
    img_dir.mkdir(parents=True, exist_ok=True)
    seed = hash(frame_id) & 0xFFFFFFFF  # unique per frame so image content differs
...
class TestDataloaderModelIntegration:
    ...
    _IMAGE_SIZE: tuple[int, int] = (16, 16)  # Small images for fast CPU tests
    ...
    def _make_dataset(
        ...
        """Write *num_frames* synthetic frames to *root* and return a Vision3DDataset."""
    ...
    def test_dataloader_batch_size_one(self, tmp_path: Path) -> None:
        """DataLoader with batch_size=1 must produce a single-frame batch that the model accepts."""
