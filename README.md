 ...
 * Bounding Box Format: Standard 10-parameter format: [x, y, z, w, l, h, \sin(\theta), \cos(\theta), v_x, v_y].
 ...
 * DetectionLoss(nn.Module): Computes Focal Loss for classification and L1/GIoU loss for the bounding box parameters [x, y, z, w, l, h, \sin(\theta), \cos(\theta), v_x, v_y].
...
This structure isolates the heavy lifting (dataset conversion, matching) from the neural network logic, making it highly testable and extensible.
Would you like me to draft the Hydra Dataclass structure (the configs/ representation in Python) next, or would you prefer to design the schema for the generic JSON dataset format?
